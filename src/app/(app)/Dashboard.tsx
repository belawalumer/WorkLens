...
import { Profile, DeveloperRole, Task, DeveloperWithData, getWorkloadStatus, Role } from '@/types'
...
export default function Dashboard({ profiles: initProfiles, roles: initRoles, tasks: initTasks, currentUserId, currentUserRole }: Props) {
  ...
  const availableCount = developers.filter(d => d.freeHours > 2 && d.role === 'developer').length
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            {developers.length} members · {availableCount} developers with free capacity · {overloadedCount} overloaded
          </p>
        </div>
      ...
      {availableCount > 0 && (
        <div className="mb-5 bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 text-sm text-blue-800 flex items-center gap-2">
          ...
          <span>
            ...
            {developers
              .filter(d => d.freeHours > 2 && d.role === 'developer')
      ...
      {/* Grid — RLS already filtered profiles to only what current user can see */}
